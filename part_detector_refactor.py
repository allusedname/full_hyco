...
def train_small_detectors(
    ...
    for part_name in parts:
        ...
        K = len(ds_train.detector_clusters.get(part_name, {}))
        if len(tr) == 0:
            ...
            if vl_loader is not None:
                ...
                with torch.no_grad(), torch.cuda.amp.autocast(enabled=(scaler.is_enabled() and (K >= 2))):
                    for xb, _, yb_val, meta in vl_loader:
                        ...
                        if K >= 2:
                            if val_has_map:
                                yb_map = _remap_val_indices_by_name(part_name, yb_val[m0], ds_val, train_name2idx)
                            else:
                                yb_map = yb_val[m0].to(device, non_blocking=True)
